...
public class UserVacantFavoriteService {
    ...
    public List<VacantDto> getAll(String username) {
    ...
    @Transactional(readOnly = true)
    public Optional<VacantDto> getById(String username, Integer id) {
        User user = userRepository.findByUsername(username);
        if(user != null){
    ...
    @Transactional
    public Boolean save(String username, Integer id) {
        User user = userRepository.findByUsername(username);
